      ...
      <div className='flex w-11/12 flex-col items-center gap-16'>
        ...
        <div className='grid w-full grid-cols-1 gap-2 sm:gap-4 md:grid-cols-3 md:gap-6 lg:gap-12'>
          {steps.map(({ number, title, desc, image }) => (
            <div key={title} className='border-muted/90 bg-muted/50 flex flex-col items-center gap-2 rounded border-2 p-2 sm:gap-4 sm:p-4 md:gap-6 lg:gap-8'>
              <span className='bg-foreground text-background flex aspect-square w-6 items-center justify-center rounded-full font-semibold sm:w-8 md:w-10 lg:w-12'>{number}</span>
              ...
              <div className='flex h-full flex-col justify-between gap-3 text-center'>
                ...
                <Image src={`/images/steps/${image}.png`} width={500} height={500} alt={`A screenshot of the ${image} screen`} className='h-full w-full rounded-sm' />
              </div>
            </div>
          ))}
        </div>
