  ...
  useEffect(() => {
    if (showOffline && reconnectAttempt < 5) {
      reconnectTimer.current = setTimeout(
        ...
        Math.min(3000 * Math.pow(2, reconnectAttempt), 30000)
      ); // Exponential backoff
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-95 backdrop-blur-sm">
      <div className="flex flex-col items-center gap-4 text-center p-8 bg-white rounded-lg shadow-xl max-w-md mx-4 animate-in fade-in duration-300">
        ...
        <button onClick={handleRetry} disabled={isReconnecting} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 disabled:cursor-not-allowed" aria-label="Retry connection">
          {isReconnecting ? (
