...
export const MacbookScroll = () => {
  ...
  return (<div ref={ref} className="flex min-h-[300vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100 md:py-80">
    <motion.h2 style={{ translateY: textTransform, opacity: textOpacity }} className="text-center text-7xl font-extrabold text-sky-50 text-shadow-lg text-shadow-sky-800">Ace JAMB with <br /> AI-assisted CBT Practice</motion.h2>
    <motion.p style={{ translateY: textTransform, opacity: textOpacity }} className="mb-10 text-center text-lg max-w-[60%] text-sky-50 text-shadow-lg text-shadow-sky-800">Access a wide range of 17 subjects, a trove of over 20,000 past questions, a realistic interface, an AI assistant, performance analytics, a leaderboard of champions, and an authentic exam experience.{' '}</motion.p>
    {/* Lid */}
...
export const Lid = ({ scaleX, scaleY, rotate, translate }: { scaleX: MotionValue<number>; scaleY: MotionValue<number>; rotate: MotionValue<number>; translate: MotionValue<number> }) => {
  return (<div className="relative [perspective:800px]">
    ...
    <motion.div style={{ scaleX: scaleX, scaleY: scaleY, rotateX: rotate, translateY: translate, transformStyle: 'preserve-3d', transformOrigin: 'top' }} className="absolute inset-0 h-96 w-[32rem] rounded-2xl bg-[#010101] p-2">
      <div className="absolute inset-0 rounded-lg bg-[#272729]" />
      <Image src="/images/hero/screenshot.png" alt="aceternity logo" fill className="absolute inset-0 h-full w-full rounded-lg object-cover object-left-top" />
    </motion.div>
  </div>);
};
...
export const Keypad = () => {
  return (<div className="mx-1 h-full [transform:translateZ(0)] rounded-md bg-[#050505] p-1 [will-change:transform]">
    {/* First Row */}
...
export const KBtn = ({ className, children, childrenClassName, backlit = true }: { className?: string; children?: React.ReactNode; childrenClassName?: string; backlit?: boolean }) => {
  return (<div className={cn('[transform:translateZ(0)] rounded-[4px] p-[0.5px] [will-change:transform]', backlit && 'bg-white/[0.2] shadow-xl shadow-white')}>
...
export const SpeakerGrid = () => {
  return (<div className="mt-2 flex h-40 gap-[2px] px-[0.5px]" style={{ backgroundImage: 'radial-gradient(circle, #08080A 0.5px, transparent 0.5px)', backgroundSize: '3px 3px' }} />);
};
