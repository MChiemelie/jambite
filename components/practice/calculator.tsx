...
const Calculator: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-4 gap-2 bg-gray-100 rounded">
      <div className="col-span-4 bg-white p-2 rounded mb-2 text-right text-2xl font-bold">{display}</div>
      {['7', '8', '9', '/', '4', '5', '6', '*', '1', '2', '3', '-', '0', '.', '=', '+'].map((btn) => (
        <Button
          ...
          className={`${btn === '=' ? 'col-span-2' : ''} ${['+', '-', '*', '/'].includes(btn) ? 'bg-blue-500 hover:bg-blue-600' : ''}`}
        >
          {btn}
        </Button>
      ))}
