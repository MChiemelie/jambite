      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 w-full text-sm md:text-md'>
        {plans.map((plan) => (
          <div key={plan.id} className='p-2 md:p-4 w-full h-96'>
            <div
              className={`h-full p-4 rounded border-2 ${plan.id === 'premium' ? 'border-accent-1' : 'border-foreground'} flex flex-col relative overflow-hidden`}
            >
              {plan.id === 'premium' && (
              ...
              <h2 className='text-xs tracking-widest title-font mb-1 uppercase'>
                {plan.name}
              </h2>
              ...
              <form action={handlePay} className='mt-auto w-full'>
                ...
                <button
                  ...
                  className='flex items-center bg-foreground text-background py-2 px-4 w-full rounded'
                >
                  {plan.buttonText}
                </button>
              </form>
