  ...
  return (
    <Dialog open={open} onOpenChange={setSubmitPopup}>
      <DialogTrigger asChild>
        <button aria-label={submitted ? 'View results' : 'End exam — open confirmation (Shortcut: S)'} className={`p-2 w-28 rounded ${submitted ? 'border-2 border-gray-300' : 'bg-red-800 text-white'}`}>
      ...
      <DialogContent className="bg-white text-black flex flex-col gap-4 w-[90%] mx-auto items-center">
        ...
        {!submitted && (
          <div className="mt-4 flex w-full justify-between">
            <button disabled={loading} className={`text-sm sm:text-base p-2 rounded w-24 sm:w-32 text-white ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-accent-3'}`} onClick={submit} aria-label="Confirm submission (Shortcut: Y)">
              {loading ? 'Submitting...' : 'Yes, Submit'}
            </button>

            <button disabled={loading} className="text-sm sm:text-base p-2 bg-accent-2 rounded w-24 sm:w-32 text-white" onClick={closePopup} aria-label="Cancel submission (Shortcut: N)">
              Cancel
            </button>
          </div>
        )}
        ...
        {submitted && (
          <div className="flex justify-center text-sm sm:text-base">
            <button className="p-2 rounded w-32 bg-accent-2 text-white" onClick={closePopup}>
              Review
            </button>
          </div>
        )}
