  ...
  return (
    <div className='sm:flex flex-col rounded p-2 shadow-sm bg-muted/50 gap-2 w-full sm:h-full'>
      <div className='flex justify-between items-center'>
        <h2 className='hidden sm:block text-md font-semibold text-gray-800 dark:text-gray-100'>
          {title}
        </h2>
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogTrigger asChild>
            <div className='flex justify-between gap-2 w-full'>
              <h3 className='hidden xs:inline sm:hidden text-md font-semibold text-gray-800 dark:text-white'>
                {value}
              </h3>
              <button
                type='button'
