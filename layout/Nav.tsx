...
export default function Nav() {
  return (
    <motion.nav className="flex justify-between items-center p-[2%] text-sm font-semibold" initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
      ...
      <Link href="/signIn" className="p-[2px] relative z-10">
        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg" />
        <div className="flex items-center justify-center space-x-2 px-8 py-[0.5rem] bg-black rounded-[6px] relative group transition duration-200 text-white hover:bg-transparent">
          <span>Sign In</span>
          <LogIn />
        </div>
      </Link>
