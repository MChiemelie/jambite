...
export default function WordComponent({ words }: { words: Word[] }) {
    ...
    <div className='rounded bg-muted/50 w-full md:min-h-full p-4 gap-4 flex flex-col justify-between'>
      <div className='flex items-center justify-between'>
        <button
          ...
          className={`flex items-center gap-2 font-semibold ${currentIndex === words.length - 1 ? 'text-gray-400' : 'text-blue-500'}`}
        ...
        <button
          ...
          className={`flex items-center gap-2 font-semibold ${currentIndex === 0 ? 'text-gray-400' : 'text-blue-500'}`}
      ...
      <p className='text-sm md:text-md lowercase overflow-auto max-h-20'>
        {currentWord.meaningHTML
          ? parse(currentWord.meaningHTML)
          : 'No meaning available.'}
      </p>
      <p className='text-sm md:text-md italic overflow-auto max-h-20'>
        {currentWord.exampleHTML
          ? parse(currentWord.exampleHTML)
      ...
      <div className='text-center'>
        ...
        {currentWord.audioUrl && (
          <audio key={currentWord.audioUrl} controls className='w-full p-1'>
            ...
            <track kind='captions' srcLang='en' label={`${currentWord.word} ${currentWord.pronunciation} ${currentWord.partOfSpeech} ${currentWord.nounType}`} />
      ...
      <div className='flex items-center justify-between border-t pt-4'>
        ...
        <Image
          width={50}
          height={50}
          src='/images/special/merriam-webster.jpeg'
