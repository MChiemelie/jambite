...
function Value({ heading, subheading, description1, description2, video, reverse, gradient }: { heading: string; subheading: string; description1: string; description2: string; video: string; reverse: string; index: number; gradient: string }) {
  return (
    <div className={cn('mx-auto flex flex-col items-center gap-4 md:flex-row md:gap-20', reverse)}>
      <div className='flex flex-col gap-2 md:max-w-lg md:gap-4'>
        <h3 className='text-md bg-gradient-to-r from-blue-800 to-indigo-900 bg-clip-text text-center font-semibold text-transparent uppercase md:text-lg lg:text-xl dark:bg-gradient-to-r dark:from-sky-400 dark:via-blue-500 dark:to-cyan-400 dark:bg-clip-text dark:text-transparent'>{heading}</h3>
        <h4 className='text-center text-2xl font-bold md:text-3xl lg:text-4xl'>{subheading}</h4>
      ...
      <div className='relative mx-auto aspect-video w-[90%] max-w-lg md:w-[70%]'>
        <div className={cn('absolute inset-0 top-5 left-5 z-0 h-full w-[4/5] rotate-[-2.5deg] rotate-y-[0.05deg] transform-gpu rounded shadow-xl transition-transform duration-2000 ease-in-out will-change-transform hover:translate-x-1 hover:translate-y-2', gradient)} />
        <div className='relative z-0 overflow-hidden rounded shadow-2xl'>
          <video width='320' height='240' src={video} autoPlay loop preload='auto' muted className='bg-muted/80 h-full w-full rotate-y-[0.05deg] transform-gpu transition-transform duration-2000 ease-in-out will-change-transform perspective-[800px]'>
