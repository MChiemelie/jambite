...
function AccuracyComponent({ data }: Props) {
  ...
  const accuracy = useMemo(
    () =>
      totalAttempts > 0
        ? Math.round((Number(correct) / totalAttempts) * 100)
        : 0,
  ...
  return (
    <Card
      className='flex flex-col justify-between bg-muted/50'
      aria-label='Accuracy card'
    >
      ...
      <CardFooter className='flex-col text-center gap-2 text-sm'>
        <p className='font-medium leading-none'>
          Your answers are {accuracy}% accurate
        </p>
