    ...
    <Dialog open={open} onOpenChange={setSubmitPopup}>
      <DialogTrigger asChild>
        <button
          ...
          className={`p-2 w-28 rounded ${submitted ? 'border-2 border-gray-300' : 'bg-red-800 text-white'}`}
        >
      ...
      <DialogContent className='bg-white text-black flex flex-col gap-4 w-[90%] mx-auto items-center'>
        <DialogHeader>
          <DialogTitle className='text-center text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold'>
            {title}
          </DialogTitle>
        ...
        {!submitted && (
          <div className='mt-4 flex w-full justify-between'>
            ...
            <button
              type='button'
              disabled={loading}
              className='text-sm sm:text-base p-2 bg-accent-2 rounded w-24 sm:w-32 text-white'
              onClick={closePopup}
        ...
        {submitted && (
          <div className='flex justify-center text-sm sm:text-base'>
            <button
              type='button'
              className='p-2 rounded w-32 bg-accent-2 text-white'
              onClick={closePopup}
            >
              Review
            </button>
