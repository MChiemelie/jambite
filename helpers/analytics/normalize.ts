...
export const toString = (v: unknown, fallback = 'Unknown') => (v == null ? fallback : String(v));

export const normalizePractice = (raw: any): Practice => {
  const data = raw.data ?? raw;
  return {
    practiceId: String(raw.$id ?? data.practiceId ?? ''),
    timestamp: toString(data.timestamp ?? data.$timestamp ?? ''),
...
export const normalizePerformance = (raw: any): Performance => {
  ...
  return {
    ...
    subject: toString(data.subject ?? 'Unknown'),
