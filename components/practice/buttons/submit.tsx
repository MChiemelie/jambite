      ...
      <DialogContent className="bg-white text-black flex flex-col gap-4 w-[90%] mx-auto items-center">
        ...
        {!submitted && (
          <div className="mt-4 flex w-full justify-between">
            <button
              className="text-sm sm:text-base p-2 bg-accent-3 rounded w-24 sm:w-32 text-white"
              onClick={() => {
                submitPractice();
              }}
            >
            ...
            <button className="text-sm sm:text-base p-2 bg-accent-2 rounded w-24 sm:w-32 text-white" onClick={() => setSubmitPopup(false)}>
              Cancel
            </button>
          </div>
        )}
