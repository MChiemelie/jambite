...
export default function WordComponent({ words }: { words: Word[] }) {
  ...
  return (
    <div className="bg-muted/50 flex w-full flex-col justify-between gap-4 rounded p-4 md:min-h-full">
      ...
      <p className="md:text-md max-h-20 overflow-auto text-sm lowercase">
        {currentWord.meaningHTML
          ? parse(currentWord.meaningHTML)
          : 'No meaning available.'}
      </p>
      <p className="md:text-md max-h-20 overflow-auto text-sm italic">
        ...
        {currentWord.audioUrl && (
          <audio key={currentWord.audioUrl} controls className="w-full p-1">
            ...
            <track
              kind="captions"
              srcLang="en"
              label={`${currentWord.word} ${currentWord.pronunciation} ${currentWord.partOfSpeech} ${currentWord.nounType}`}
            />
