...
import { ReportQuestionProps } from '@/types';

const ReportQuestionDialog: React.FC<ReportQuestionProps> = ({ subject, questionId, fullName = '', onReportSubmitted }) => {
  const [open, setOpen] = useState(false);
  const [reportType, setReportType] = useState<number>(1); // default: Question
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="flex items-center gap-2 text-gray-700 hover:text-gray-900 p-2 rounded border-2 border-gray-400 hover:border-gray-800">
          <Flag />
      ...
      <DialogContent className="sm:max-w-[625px] bg-white text-black">
        <DialogHeader>
          <DialogTitle className="text-center text-2xl">Report Question</DialogTitle>
