      ...
      <div className='w-11/12 flex flex-col items-center gap-16'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4 md:gap-6 lg:gap-12 w-full'>
          {steps.map(({ number, title, desc, image }) => (
            <div
              key={title}
              className='flex flex-col items-center gap-2 sm:gap-4 md:gap-6 lg:gap-8 border-2 border-muted/90 p-2 sm:p-4 rounded bg-muted/50'
            >
              <span className='aspect-square w-6 sm:w-8 md:w-10 lg:w-12 flex items-center justify-center rounded-full bg-foreground text-background font-semibold'>
                {number}
              </span>
