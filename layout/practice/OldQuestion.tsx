...
export default function Question({ session, questionsData }: { session: Session | null; questionsData: QuestionsProps }) {
  ...
  return (
    <div className="py-2">
      ...
      {submitted && (
        <p className="text-center text-md lg:text-xl my-4">
          You scored {score} out of {questionsData.data.length}!{' '}
        </p>
      )}
      <div className="flex justify-between w-3/5 mx-auto my-5 text-md lg:text-xl">
        <button aria-label="Previous Question Button" onClick={handlePreviousQuestion} disabled={currentQuestion === 0} className={`${styles.button} bg-accent-1`}>
          Previous
        </button>
        ...
        <button aria-label="Next Question Button" onClick={handleNextQuestion} disabled={currentQuestion === questionsData.data.length - 1} className={`${styles.button} bg-accent-3`}>
          Next
        </button>
      </div>
      <div className="flex flex-wrap justify-center w-4/5 mx-auto my-5 gap-1">
        {questionsData.data.map((question, index) => (
          <span key={question.id} className={`p-1 rounded-full text-sm text-center ${selectedAnswers[question.id] !== undefined ? 'bg-accent-1' : 'bg-accent-2'}`} onClick={() => setCurrentQuestion(index)}>
            {index + 1}
          </span>
        ))}
      </div>
